...
def _connect_internal(
    ...
    if not connected.is_set():
        ws.close()
        raise Exception(
            f"Could not connected to Pglet server in {constants.CONNECT_TIMEOUT_SECONDS} seconds."
        )
...
def _download_pglet():
    ...
    if pglet_path.exists():
        ...
        installed_ver = subprocess.check_output([str(pglet_path), "--version"]).decode(
            "utf-8"
        )
        ...
        try:
            ...
            if is_windows():
                ...
                with tarfile.open(temp_arch, "r:gz") as tar_arch:
                    ...
                    safe_extract(tar_arch, pglet_bin)
        finally:
            os.remove(temp_arch)
